...
angular.module('ZrsmWorker')
    .factory('workOnService',function($window,$cookies,$http,$q,$state,API_CONFIG,$ionicPopup){
        var overtimelist = function(){
            var overtimelist_api = API_CONFIG.defaultPath + '/overtimelist';
            var deferred = $q.defer();
            $http.post(overtimelist_api,API_CONFIG.paramsHandler({},$cookies)).then(function (res) {
                if (res.status === 204) {
                    console.log('登出')
        ...
        var commitovertime = function(param) {
            ...
            var deferred = $q.defer();
            $http.post(commitovertime_api,API_CONFIG.paramsHandler(param,$cookies)).then(function (res) {
                if (res.status === 204) {
                    console.log('登出')
                    deferred.reject(null);
                } else if (res.status === 200) {
